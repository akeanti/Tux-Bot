...
class TicketConfig:
    def __init__(self):
        ...
        self.allowed_roles = []
        self.panel_custom = {}  # Dictionary to hold custom panels for different users
...
class TicketModal(Modal):
    def __init__(self, bot):
        ...
        self.bot = bot
        self.reason = TextInput(label="Reason for opening this ticket", style=discord.TextStyle.long, required=True)
        self.add_item(self.reason)

    async def on_submit(self, interaction: discord.Interaction):
        await create_ticket_channel(interaction, self.reason.value, self.bot)

async def create_ticket_channel(interaction: discord.Interaction, reason: str, bot):
    ...
    existing_ticket = discord.utils.get(guild.text_channels, name=f"ticket-{interaction.user.name}")
    if existing_ticket:
        await interaction.response.send_message("You already have an open ticket.", ephemeral=True)
        return

    # Check if the server has reached the ticket limit
    ticket_channels = [ch for ch in guild.text_channels if ch.name.startswith("ticket-")]
    if len(ticket_channels) >= 50:
        await interaction.response.send_message("The server has reached the maximum number of tickets.", ephemeral=True)
        return
...
class TicketConfigModal(Modal):
    def __init__(self):
        ...
        self.title = TextInput(label="Panel Title", default=config.embed_title, style=discord.TextStyle.short)
        self.description = TextInput(label="Panel Description", default=config.embed_description, style=discord.TextStyle.long)
        self.color = TextInput(label="Embed Color (hex)", default=f"#{config.embed_color.value:06x}", style=discord.TextStyle.short)
        self.image = TextInput(label="Embed Image URL", default=config.embed_image, style=discord.TextStyle.short)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            config.embed_color = discord.Color(int(self.color.value.strip("#"), 16))
        except ValueError:
            await interaction.response.send_message("Invalid color code. Using default color.", ephemeral=True)
            config.embed_color = discord.Color.green()
        config.embed_image = self.image.value

        await interaction.response.send_message("Ticket panel configuration updated.")
...
class Ticket(commands.Cog):
    ...
    @discord.app_commands.command(name='ticket', description='Create a ticket panel.')
    ...
    @discord.app_commands.command(name='close', description='Close an open ticket.')
    async def close_ticket(self, interaction: discord.Interaction):
        """Closes an open ticket."""
        ticket_channel = discord.utils.get(interaction.guild.text_channels, name=f"ticket-{interaction.user.name}")
        if ticket_channel is None:
            await interaction.response.send_message("You do not have an open ticket.")
            return

        # Log the closure
        log_channel = discord.utils.get(interaction.guild.text_channels, name="ticket-logs")
        if log_channel is None:
            log_channel = await interaction.guild.create_text_channel(name="ticket-logs")
    ...
    @discord.app_commands.command(name='setpanel', description='Set the ticket panel channel.')
    async def set_panel(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"Ticket panel channel set to {interaction.channel.mention}.")

    @discord.app_commands.command(name='setroles', description='Set the roles that can access tickets.')
    async def set_roles(self, interaction: discord.Interaction, roles: discord.Role):
        """Set the roles allowed to access tickets."""
        config.allowed_roles.append(roles)
        role_mentions = ', '.join([role.mention for role in config.allowed_roles])
        await interaction.response.send_message(f"Roles allowed to access tickets: {role_mentions}.")

    @discord.app_commands.command(name='ticket-help', description='Display all ticket-related commands.')
    ...
    @discord.app_commands.command(name='ls-tickets', description='List all open tickets.')
    async def ls_tickets(self, interaction: discord.Interaction):
        ...
        if not ticket_channels:
            await interaction.response.send_message("There are no open tickets.")
            return

        ticket_list = "\n".join([f"{ch.name}: {ch.mention}" for ch in ticket_channels])
        await interaction.response.send_message(f"Open tickets:\n{ticket_list}")

    @discord.app_commands.command(name='rm-ticket', description='Remove a specific ticket by name.')
    async def rm_ticket(self, interaction: discord.Interaction, ticket_name: str):
        """Removes a specific ticket by name."""
        ticket_channel = discord.utils.get(interaction.guild.text_channels, name=ticket_name)
        if ticket_channel is None:
            await interaction.response.send_message("No ticket found with that name.")
            return

        await ticket_channel.delete()
        await interaction.response.send_message(f"Ticket '{ticket_name}' has been removed.")
